	...
	cfg := &HyperdriveConfig{
		...
		Network: config.Parameter[config.Network]{
			ParameterCommon: &config.ParameterCommon{
				...
				AffectsContainers:  []config.ContainerID{config.ContainerID_Daemon, config.ContainerID_ExecutionClient, config.ContainerID_BeaconNode, config.ContainerID_ValidatorClient, pbs.ContainerID_Pbs},
				CanBeBlank:         false,
				OverwriteOnUpgrade: false,
			},
